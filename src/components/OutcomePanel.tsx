 ...
 *
 * Previously supported "Outcome All" — a toggle (plus a chip row showing
 * which channels it applied to) letting one Approve & Save cover every open
 * channel on a card at once, for the now-removed "Elevation" collapsed-card
 * view. Removed per an explicit follow-up: only one channel is ever
 * dispositioned at a time now, so `OutcomeForm` has no multi-channel concept
 * left at all — `onApprove` just reports the outcome itself. */
...
export interface OutcomeButtonProps {
  ...
  customerName: string;
  /** Controlled, not internal — lifted up so a future consumer could
   *  coordinate against it if needed; currently just passed straight
   *  through. The trigger button below still owns its own click
   *  handling/position capture, it just reports the open request upward
   *  instead of flipping local state. */
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** Only one channel is ever being dispositioned at a time — see this
   *  file's own note on why "Outcome All" was removed. */
  onApprove?: (outcome: OutcomeResult) => void;
}
...
export function OutcomeButton({ customerName, open, onOpenChange, onApprove }: OutcomeButtonProps) {
